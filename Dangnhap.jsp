...
<%
    ...
    if ("POST".equalsIgnoreCase(request.getMethod())) {
        ...
        Khachhang kh = dao.dangNhapBangEmailHoacTenDangNhap(email, matkhau);
        if (kh != null) {
            session.setAttribute("khachhang", kh);
            response.sendRedirect("Trangchu.jsp");
            return;
        }
        loi = "Sai email / tên đăng nhập hoặc mật khẩu.";
    }
    ...
    <div class="page-outer">
        ...
        <div class="site-shell login-page-wrap">
            <div class="login-page-inner">
                ...
                <p class="login-page-sub">Bạn chưa có tài khoản? <a href="Dangky.jsp">Đăng ký tại đây</a></p>
                ...
                <div class="login-card-wide">
                    ...
                    <% } %>
                    <% if ("1".equals(reset)) { %>
                    <div class="auth-msg auth-msg--ok">Đổi mật khẩu thành công. Vui lòng đăng nhập lại.</div>
                    <% } %>
                    <% if ("need_config".equals(oauth)) { %>
                    <div class="auth-msg auth-msg--err">Chưa cấu hình OAuth. Sửa App ID / Secret trong <code>WEB-INF/web.xml</code>. Redirect URI (đăng ký tại Facebook/Google): <br><code>http://<%= request.getServerName() %>:<%= request.getServerPort() %><%= ctx %>/oauth/callback</code></div>
                    <% } %>
                    <% if ("cancel".equals(oauth)) { %>
                    <div class="auth-msg auth-msg--err">Bạn đã hủy đăng nhập OAuth.</div>
                    <% } %>
                    <% if ("invalid".equals(oauth) || "fail".equals(oauth)) { %>
                    <div class="auth-msg auth-msg--err">Đăng nhập OAuth không thành công. Thử lại hoặc dùng email.</div>
                    <% } %>
                    <% if (!loi.isEmpty()) { %>
                    <div class="auth-msg auth-msg--err"><%= loi %></div>
                    <% } %>

                    <form class="auth-form" method="post" action="Dangnhap.jsp" autocomplete="on">
                        ...
                        <a class="login-forgot login-forgot--btn" href="Quenmatkhau.jsp">Quên mật khẩu</a>

                        <div class="auth-actions">
                            <button type="submit" class="login-submit-yellow">Đăng nhập</button>
                        </div>
                    </form>

                    <p class="social-login-title">Hoặc đăng nhập bằng</p>
                    <div class="social-buttons">
                        <a class="social-btn social-btn--fb" href="<%= ctx %>/oauth/login?provider=facebook">
                            <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v2.385z"/></svg>
                            Facebook
                        </a>
                        <a class="social-btn social-btn--gg" href="<%= ctx %>/oauth/login?provider=google">
